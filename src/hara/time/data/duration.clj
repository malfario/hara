...
(defn adjust-days
  ...
  (if-not (and year month day)
    (throw (Exception. (format "Year (%s), Month (%s) and Day (%s) are required:" year month day))))
  (let [year-days  (adjust-year-days years rep)
...
(defmethod time/-from-length :default
  [long _]
