(ns documentation.hara-sort)

[[:chapter {:title "Introduction"}]]

"
[hara.sort](https://github.com/zcaudate/hara/blob/master/src/hara/sort.clj) contain functions and utilities for strings."

[[:section {:title "Installation"}]]

"
Add to `project.clj` dependencies:

    [im.chit/hara.sort \"{{PROJECT.version}}\"]

Individual namespaces can be added seperately:

    [im.chit/hara.sort.topological \"{{PROJECT.version}}\"]
"

[[:chapter {:title "API - topological"}]]

"[hara.sort.topological](https://github.com/zcaudate/hara/blob/master/src/hara/sort/topological.clj)"

[[:api {:namespace "hara.sort.topological"}]]
